...
describe('client', () => {
  it('takes a request, sets up a channel to the parent window, and promises a response from the parent', async () => {
    ...
    const request = (0, _ipc.client)(parent, origin);
...
describe('server', () => {
  it('only responds to valid requests', done => {
    ...
    const s = (0, _ipc.server)(child, child.location.origin)(handler).start();
  ...
  it('delegates to a handler to compute responses', done => {
    expect.assertions(3);
    const testHandler = jest.fn(request => Promise.resolve({
      id: request.id,
      ret: 'testHandler return value'
    }));
    const parent = window;
    const child = parent;
    const s = (0, _ipc.server)(child, child.location.origin)(testHandler).start();
